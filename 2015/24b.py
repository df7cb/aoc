...
# check if a c2/c3/c4 partition with a given c1 exists
def load2(c1, c2, loading, p):
    for p in range(p, len(packages)):
        ...
        if loading + packages[p] < compartment:
            ...
            if load3(c1+c2+[packages[p]], [], 0, 0):
                return True
        else:
            break
    return False

# check if a c3/c4 partition with a given c1+c2 exists
def load3(c12, c3, loading, p):
    for p in range(p, len(packages)):
        if packages[p] in c12:
            continue
        if loading + packages[p] < compartment:
            if load3(c12, c3 + [packages[p]], loading + packages[p], p+1):
                return True
        elif loading + packages[p] == compartment:
            print(c12, c3 + [packages[p]])
            return True
        else:
            break
    return False
