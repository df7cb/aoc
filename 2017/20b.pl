...
my %part;
while (<DATA>) {
	die unless /p=<(-?\d+),(-?\d+),(-?\d+)>, v=<(-?\d+),(-?\d+),(-?\d+)>, a=<(-?\d+),(-?\d+),(-?\d+)>/;
	$part{$n++} = { p=>[$1, $2, $3], v=>[$4, $5, $6], a=>[$7, $8, $9] };
}
...
sub check_collisions
{
	my %collider;
	my %collision;
	foreach my $n (keys %part) {
		my @p = @{$part{$n}->{p}};
		my $pkey = "$p[0],$p[1],$p[2]";
		if (exists $collider{$pkey}) {
			print "particle $n collides with $collider{$pkey} at $pkey\n";
			$collision{$n} = 1;
			$collision{$collider{$pkey}} = 1;
		}
		$collider{$pkey} = $n;
	}
	foreach my $n (keys %collision) {
		print "particle $n was hit\n";
		delete $part{$n};
	}
	if (keys %collision) {
		my $num = keys %part;
		print "$num particles left\n";
	}
}
