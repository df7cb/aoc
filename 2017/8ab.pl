...
while (<DATA>) {
	die $_ unless /(\S+) (inc|dec) (-?\d+) if (\S+) (<|<=|==|!=|>=|>) (-?\d+)/;
	my ($reg, $op, $const, $condreg, $cond, $condconst) = ($1, $2, $3, $4, $5, $6);
	$const = -$const if $op eq 'dec';
	$reg{$reg} //= 0;
	$reg{$condreg} //= 0;
	if (eval "$reg{$condreg} $cond $condconst") {
		$reg{$reg} += $const;
	}
	$max = $reg{$reg} if $reg{$reg} > $max;
}

my $maxreg;
foreach my $reg (keys %reg) {
	$maxreg //= $reg;
	$maxreg = $reg if $reg{$reg} > $reg{$maxreg};
}

print "max: $max\n";
print "$maxreg: $reg{$maxreg}\n";
